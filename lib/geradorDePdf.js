...
var path = require('path'),
    ...
    barcode128c = gammautils.barcode.barcode128c,
...
module.exports = function(danfe, args, callback) {
    ...
    function linhaVertical(y1, y2, x, cor) {
        ...
        return pdf.moveTo(x, y1).lineTo(x, y2).stroke(cor || args.corDoLayout);
    }
    ...
    function normal(string, x, y, largura, alinhamento, tamanho) {
        pdf.font('normal')
            .fillColor(args.corDoTitulo)
            .fontSize(tamanho || 8)
            .text(string,
    ...
    normal([
        'Recebemos de',
        'Gammasoft Desenvolvimento de Software Ltda', //Emissor
        'os produtos e/ou serviços constantes da nota',
        'fiscal eletrônica indicada abaixo.',
        'Emissão:',
        '11/08/2014', //Data de emissão
        '- Valor Total:',
        'R$ 230,76', //Valor total
        '- Destinatário:',
        'Valemar - Vale do Maranhão Mineração Ltda', //Destinatario
        '- Endereço:',
        'Rod. GO 230 KM 06 A ESQUERDA 26 KM, S/N - Faz. Folha Larga Zona Rural Mimoso de Goiás/GO' //Endereco
    ].join(' ').toUpperCase(), 1.5, 3, 472.5, 'justify', 6.9);

    titulo('DANFE', 266.5, 63.5, 197, 'left', 14);
    normal('NF-e', 476.6, 1.8, 110, 'center', 14);
    normal('Nº. 000.000.002', 476.6, 22, 110, 'center', 10);
    normal('Série 001', 476.6, 31.5, 110, 'center', 10);

    italico('IDENTIFICAÇÃO DO EMITENTE', 1, 60, 238);

    var temLogotipo = true,
    ...
    if(temLogotipo) {
        var caminhoDoLogotipo = path.join(__dirname, '../tests/logotipo.png'); //TODO: Pegar do objeto DANFE
        pdf.image(caminhoDoLogotipo, margemEsquerda + args.ajusteX + 4.5, margemTopo + args.ajusteY + args.ajusteYDoLogotipo + 78, {
            fit: [60, 60]
        });
    }

    negrito('Gammasoft Desenvolvimento de Software Ltda', identificacaoDoEmitenteX, identificacaoDoEmitenteY + args.ajusteYDaIdentificacaoDoEmitente, identificacaoDoEmitenteLargura, 'center', 8 + identificacaoDoEmitenteFonte);
    normal('SIA Trecho 03 Lotes 625/695 Bloco A, Sala 333',
        identificacaoDoEmitenteX,
        pdf.y - margemTopo + 2,
    identificacaoDoEmitenteLargura, 'center', 6 + identificacaoDoEmitenteFonte);

    normal('Guará, Brasília/DF — 72.530-070',
        identificacaoDoEmitenteX,
        pdf.y - margemTopo,
    identificacaoDoEmitenteLargura, 'center', 6 + identificacaoDoEmitenteFonte);

    normal('Telefone: (61) 8633-3051',
        identificacaoDoEmitenteX,
        pdf.y - margemTopo + 2,
    identificacaoDoEmitenteLargura, 'center', 6 + identificacaoDoEmitenteFonte);

    normal('Email: contato@gammasoft.com.br',
        identificacaoDoEmitenteX,
        pdf.y - margemTopo,
    identificacaoDoEmitenteLargura, 'center', 6 + identificacaoDoEmitenteFonte);
    ...
    negrito('1', 317.5, 96.8, 14.5, 'center', 18);

    normal('Consulta de autenticidade no portal nacional da NF-e', 340.5, 128, 244);
    normal('www.nfe.fazenda.gov.br/portal ou no site da Sefaz Autorizadora', 340.5, 139.5, 244);

    normal('Nº. 000.000.002', 241.2, 121.2, 98.5, 'center', 10);
    normal('Série 001', 241.2, 131.2, 98.5, 'center', 10);
    italico('Folha 1/1', 241.2, 141.2, 98.5, 'center', 8); //Vai estar dentro do forEach

    //RECIBO
    titulo('DATA DE RECEBIMENTO', 1.5, 29, 97);
    titulo('IDENTIFICAÇÃO E ASSINATURA DO RECEBEDOR', 100.5, 29, 374);

    var codigoDeBarras = barcode128c('53140819950366000150550010000000021598008207');
    pdf.font('codigoDeBarras').fontSize(32).text(codigoDeBarras.asciiString, margemEsquerda + 339, margemTopo + 65, {
        align: 'center',
        width: 249
    });
    ...
    //SEXTO BLOCO
    secao('CÁLCULO DO ISSQN', 1.5, 755);
    titulo('INSCRIÇÃO MUNICIPAL', 1.5, 763, 132.5);
    campo('0767608200153', 1.5, 771, 132.5, 'left');

    titulo('VALOR TOTAL DOS SERVIÇOS', 137.5, 763, 132.5);
    campo('230,76', 137.5, 771, 132.5, 'right');

    titulo('BASE DE CÁLCULO DO ISSQN', 273.5, 763, 132.5);
    campo('230,76', 273.5, 771, 132.5, 'right');

    titulo('VALOR TOTAL DO ISSQN', 409.5, 763, 175.5);
    campo('4,62', 409.5, 771, 175.5, 'right');

    secao('DADOS ADICIONAIS', 1.5, 786);
    titulo('INFORMAÇÕES COMPLEMENTARES', 1.5, 794.5, 385);
    titulo('RESERVADO AO FISCO', 390, 794.5, 195);
