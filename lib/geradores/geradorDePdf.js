...
module.exports = function(danfe, args, callback) {
    ...
    function desenharPagina() {
        ...
        normal(danfe.getInformacoesComplementares(), 1, 801, 386, 'justify', 6);
        ...
        normal(danfe.getNumeroFormatado(), 241.2, 121.2, 98.5, 'center', 10);
        normal(danfe.getSerieFormatada(), 241.2, 131.2, 98.5, 'center', 10);
        ...
        pdf.font('codigoDeBarras').fontSize(32).text(codigoDeBarras.asciiString, args.ajusteX + margemEsquerda + 339, args.ajusteY + margemTopo + 65, {
            align: 'center',
            width: 249
        });
        ...
        campo(transportador.getPlacaDoVeiculo(), 347.5, 329, 84);
        ...
        campo(emitente.getInscricaoEstadual(), 1.5, 771, 132.5, 'left');
    ...
    var yDoItemAtual = 406;
    itens.forEach(function(item) {
        normal(item.getCodigo(), 1.5, yDoItemAtual, 51, 'center', args.tamanhoDaFonteDosItens);
        var maiorY = pdf.y;

        normal(item.getDescricao(), 55.5, yDoItemAtual, 179, 'left', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getNcmSh(), 235.5, yDoItemAtual, 32.5, 'center', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getOCst(), 270, yDoItemAtual, 21, 'center', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getCfop(), 293.5, yDoItemAtual, 21, 'center', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getUnidade(), 315, yDoItemAtual, 16.5, 'center', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getQuantidadeFormatada(), 332.5, yDoItemAtual, 37.5, 'right', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getValorUnitarioFormatado(), 372, yDoItemAtual, 32.5, 'right', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getValorTotalFormatado(), 407, yDoItemAtual, 31, 'right', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getBaseDeCalculoDoIcmsFormatada(), 440, yDoItemAtual, 32.5, 'right', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getBaseDeCalculoDoIcmsFormatada(), 440, yDoItemAtual, 32.5, 'right', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getValorDoIcmsFormatado(), 474.5, yDoItemAtual, 32, 'right', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getValorDoIpiFormatado(), 508.5, yDoItemAtual, 26, 'right', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getAliquotaDoIcmsFormatada(), 535, yDoItemAtual + 0.75, 20, 'center', args.tamanhoDaFonteDosItens - 1);//
        maiorY = Math.max(maiorY, pdf.y);

        normal(item.getAliquotaDoIpiFormatada(), 556.5, yDoItemAtual, 29, 'center', args.tamanhoDaFonteDosItens);
        maiorY = Math.max(maiorY, pdf.y);
