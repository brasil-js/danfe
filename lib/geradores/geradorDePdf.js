...
var pdfDefaults = {
    ...
    ambiente: 'producao',
    opcidadeDaHomologacao: 0.2,
    ajusteYDaHomologacao: 0,
    ...
    function desenharPagina() {
        if(args.ambiente !== 'producao') {
            pdf.font('normal')
                .fillColor(args.corDoTitulo)
                .fontSize(50)
                .fillOpacity(args.opcidadeDaHomologacao)
                .text('HOMOLOGAÇÃO',
        ...
        function codigo128(text) { //TODO: Corrigir no gammautils
            if(text.length % 2 !== 0) {
            ...
            return text.match(/.{2}/g).map(Number).reduce(function(acc, value, index){
        ...
        if(danfe.getChaveDeAcesso()) {
            pdf.font('codigoDeBarras')
                ...
                .text(codigo128(danfe.getChaveDeAcesso()), args.ajusteX + margemEsquerda + 339, args.ajusteY + margemTopo + 65, {
                    align: 'center',
                    width: 249
                });
        }
